...
for k = 1: size(query_fns, 1)
    ...
    fprintf(fp, '<td><img src="%s" alt="Query" style="margin-right: 20px; width: 64px; height: 64px; border-right: solid 3px red;" /><h3>%s</h3></td>', qimg, qobj_id);
    prev_res_id = '';
    obj_num = 0;
    i = 1;
    while obj_num < topK
        ...
        fprintf(fp, '<td><img src="%s" alt="%f" style="width: 64px; height: 64px"/><h3>%s</h3></td>', res_file, 0, robj_id);
        obj_num = obj_num + 1;
        prev_res_id = robj_id;
    end
